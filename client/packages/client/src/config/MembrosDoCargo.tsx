  ...
  */
  ...
  function executar(ids: readonly string[], dar: boolean) {
    ...
    void aplicarCargoEmLote(serverId, cargo.id, ids, dar, (terminados, total) => {
      ...
      /*
        ⚠ **`membrosDoServidor` e não `ids`** — o parâmetro desta função se
        chama `ids` (os alvos do lote) e SOMBREAVA a lista do servidor. A
        releitura saía só com as pessoas alteradas, e a coluna de cargos
        passava a contar 3 · 0 · 1 onde havia 4 · 6 · 8. Medido no navegador.
      */
      const relidas = pessoasDoServidor(serverId, membrosDoServidor);
      setPessoas(relidas);
      /*
