...
describe("escolherAutor", () => {
  const pessoas = [
    { id: "M", nomes: ["marina", "Marina Alcântara"] },
    { id: "T", nomes: ["teo", "Téo Nakamura"] },
    { id: "A", nomes: ["mariana", "Mariana Souza"] },
  ];

  it("nome inteiro ganha, sem acento nem caixa", () => {
    expect(escolherAutor("Téo", pessoas)).toBe("T");
    expect(escolherAutor("marina", pessoas)).toBe("M");
  });

  it("palavra do nome serve, se for uma pessoa só", () => {
    expect(escolherAutor("naka", pessoas)).toBe("T");
  });

  it("prefixo ambíguo não escolhe ninguém", () => {
    expect(escolherAutor("mar", pessoas)).toBeUndefined();
    expect(escolherAutor("", pessoas)).toBeUndefined();
  });
});
