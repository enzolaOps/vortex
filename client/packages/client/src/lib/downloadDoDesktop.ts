 ...
 * com `Vortex-Setup.exe`, `Vortex.deb` e `Vortex.pkg.tar.zst`, e
 * `releases/latest/download/` redireciona para a última release publicada —
 * o link não precisa mudar a cada versão.
 *
 * Linux: Ubuntu/Debian/Mint recebem o `.deb`; Arch (e o resto que não é
 * Debian), o pacote pacman. Android se anuncia como Linux e não instala nenhum.
 *
...
export function linkDeDownload(plataforma: string): string {
  ...
  if (p.includes("linux")) {
    ...
    return `${RELEASES}/download/Vortex.pkg.tar.zst`;
  }
  return RELEASES;
}
