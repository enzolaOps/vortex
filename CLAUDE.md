...
| Primitivos: seções 6 e 7 pendentes | **Abas · segmento · tooltip** e **banner · estado vazio**, as duas últimas da matriz da referência. `Tabs` e `Banner` não existem como peça aqui; `Segmentado`, `Tooltip` e `EstadoVazio` existem e passaram só pelo passe de tamanho, não pela comparação de estados. |
