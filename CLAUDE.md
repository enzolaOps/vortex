...
Levantado de `vendor/stoat-web/packages/client`. Inventário do que vai ser substituído — e,
...
Três no upstream, todos em `vendor/stoat-web/packages/`:
...
**Existe agora um quarto gitlink:** `client/packages/stoat.js`. Ele é a
dependência do produto; os três gitlinks em `vendor/stoat-web/` pertencem à
referência e não são gates de CI do produto.

### Onde o código está

```text
vendor/stoat-web/packages/client/
...
| Qualidade de voz — o adjetivo E os milissegundos | **Os dois agora, e a decisão antiga continua de pé.** Ela recusava derivar "42 ms" de `ConnectionQuality`, que é CLASSIFICAÇÃO (`excellent/good/poor/lost`) — e continua recusando. O que entrou na faixa é o RTT MEDIDO: `estatisticasDeVoz` lê `currentRoundTripTime` do par de candidatos NOMINADO do `RTCStatsReport`, que existe de verdade, e até agora só chegava ao overlay de debug. O adjetivo responde "dá para falar?" e o número responde "quanto?"; quem decide trocar de rede quer os dois. ⚠ **Componente próprio (`Rtt`), e ele é o único da faixa que acorda por segundo** — pôr o `setInterval` na `FaixaDeVoz` repintaria destino, medidor e os cinco botões a cada amostra, e pôr o número no store da chamada acordaria o cartão e a coluna junto. É a mesma separação do `Cronometro`. Ausente enquanto não há amostra: "0 ms" seria o melhor caso possível escrito exatamente quando nada foi medido. Estado de CONEXÃO segue ganhando — durante "entrando…" e "reconectando…" o RTT não aparece. |
...
| Brand assets | **Resolvido.** `brand/` é diretório rastreado deste repo, não submodule: `mark.svg`, `wordmark.svg`, `monochrome.svg` + `generate.mjs`. `client/` consome o SVG diretamente; os rasters são mantidos apenas nas referências em `vendor/`. |
...
| Teste de navegador | **vitest instalado**, 19 testes cobrindo store, adapter e toast. Falta runner de NAVEGADOR: jsdom não tem engine de layout, então âncora, remedição e o firehose seguem medidos à mão no arnês. A referência `vendor/stoat-web/` já usa Playwright — é o candidato natural. |
...
| Modo compacto da timeline | **CONSTRUÍDO — esta linha dizia "não construído, e é a maior divergência que sobra", e envelheceu.** Medido na árvore atual: `MessageRow` referencia a densidade em 14 pontos, `MessageList` em 19, `ALTURA_POR_TIPO` é POR DENSIDADE com a razão escrita (*"sem isto a âncora quebraria no modo novo"*), e 19 arquivos consomem o store, o de sincronização inclusive. Conferido contra o design, que pede quatro coisas: sem avatar ✓, hora em mono ✓ (JetBrains Mono 11px, coluna de 40, `text-align: end`, `tabular-nums`), sem agrupamento por autor ✓ (`abreGrupo` e `continua` iguais de propósito), anexo sem `transform` ✓. **Seis de sete medidas exatas.** ⚠ A sétima: entrelinha em 1,50 contra 1,45 do design — é `leading-relaxed` do Tailwind, e o degrau 1,45 não existe na escala. |
...
| Selo `LIVE` na sala de voz | **CONSTRUÍDO, e a linha anterior dizia que era falta de DADO — metade disso segue verdade.** O selo existe e acende por `UserVoiceState.screensharing`, com os valores do design (`#F0808D` sobre vermelho a 14%, raio 4, 700), no lugar do glifo `◧`: numa coluna de 232px `LIVE` se lê sem consultar a legenda e o glifo não. ⚠ **O que ele afirma é TRANSMITINDO, não "com espectadores"** — a legenda do design escreve a segunda coisa, e a contagem de espectadores continua não existindo: `UserVoiceState` tem `screensharing` e nada sobre quem assiste, `livekit-client` só conhece as assinaturas da PRÓPRIA conexão, e o motor usa `autoSubscribe: false` (faixa de tela nunca é assinada). Contar espectadores segue sendo webhook do LiveKit → serviço `api` → campo novo. A divergência é de SIGNIFICADO e fica escrita; a alternativa era um selo que nunca acende. |
...
| `warningSubtle` no `Botao` | **CONSTRUÍDO — esta linha dizia "fora por falta de consumidor", e envelheceu.** `VarianteDeBotao` tem as seis da referência: `primario`, `neutro`, `sutil`, `perigo`, `perigoSutil` e `avisoSutil`. ⚠ **O que sobrou do mesmo problema mudou de componente:** `BotaoDeIcone` tem quatro tons (`padrao`, `acento`, `perigo`, `sucesso`) contra os cinco da referência — falta `aviso`, e a família existe em `Botao`, `Banner` e `Selo`. Um botão de ícone que precise dizer "atenção" hoje não tem como. |
...
| Pill de cargo | **CONSTRUÍDO — esta linha dizia "fora por falta de DADO", e envelheceu.** `membros/PilulasDeCargo.tsx` existe e consome `cargosDoServidor` e `useCorDeCargo`, ou seja a tabela de cargos resolvida que a linha antiga dava como ausente chegou. A geometria medida na referência que ficou anotada aqui (`pad 0 10 0 6`, gap 6, pílula, fundo na cor a 15%, 12/600, ponto de 6px) já serviu ao que foi construído. |
...
| ⚠ O app falava com `https://stoat.chat/api` | **O bloqueio raiz de TUDO que depende de rede, e ele não dava erro nenhum.** `new Client()` era construído sem `baseURL`, e o default do SDK é a instância PÚBLICA do Stoat — então login, criar conta, recuperar senha e todo `fetch` do adapter iam para o servidor de outra gente. Uma senha digitada na tela de entrada teria saído daqui. `sdk/config.ts` resolve a API contra `location.origin`, que é o que o compose descreve (`REVOLT__HOSTS__API` = `https://$DOMINIO/api`, cliente no mesmo domínio). ⚠ **E NÃO o `/api` relativo do cliente Solid:** o `stoat-api` monta a requisição com `new URL(path)` SEM base, então caminho relativo lança `Invalid URL` antes de qualquer `fetch` — a suíte acusou com 18 rejeições no `#fetchConfiguration`. Medido depois: uma única chamada, para `localhost:4174/api/?`. |
| A suíte de testes saía para a internet | ⚠ **Descoberto ao consertar o `baseURL`, e o problema era anterior.** O construtor do `Client` dispara `#fetchConfiguration()` sem `await`; com o default apontando para `stoat.chat`, toda corrida de `pnpm test` que importasse qualquer módulo tocando o `client` buscava configuração na instância pública — e PASSAVA, porque a chamada tinha sucesso. Ruim por três razões independentes: a suíte falhava sem internet, o tempo dela dependia de servidor de terceiro, e o repositório anunciava cada corrida para fora. O erro só apareceu quando `localhost` substituiu `stoat.chat` e o mesmo `fetch` passou a falhar. `src/testes/documento.ts` agora dubla `fetch` com uma configuração mínima. ⚠ `Response` do Node e não `janela.Response`: o do jsdom não é construtor, e a primeira tentativa trocou 18 rejeições de rede por 18 de `not a constructor`. |
| Enviar mensagem nunca chamou o servidor | ⚠ **A maior ausência funcional do projeto, e ela estava documentada como decisão.** `enviarMensagem` criava a mensagem local e um `setTimeout` a marcava "enviada" — o caminho de rede simplesmente não existia. O comentário justificava com o ID que muda entre a otimista e a confirmada, e essa razão **expirou**: a reconciliação por nonce foi construída, com doze testes. Agora há `channel.sendMessage({content, nonce, replies, attachments})` de verdade, e a simulação virou `{ativa: true}`, ligada só pelo arnês — simular E enviar mandaria a mensagem duas vezes. ⚠ **O POST não marca `sent`:** quem materializa a confirmada é o `messageCreate` que chega pelo socket com o nonce; marcar aqui correria com ele e a linha piscaria. Sucesso é a ausência de falha. ⚠ **`replies` vai com `mention: false`** — enquanto `responderSemMencionar` não existe, o inverso transformaria toda resposta numa menção que ninguém pediu. |
| Avatar e ícone reais | **A imagem COBRE o gradiente, nunca o substitui.** Ela vem do servidor de mídia e pode demorar ou falhar; com o gradiente por baixo, o intervalo entre montar e carregar mostra a identidade de sempre, e falha de carregamento cai no fallback sozinha, sem estado. Ligada em avatar de pessoa (13 superfícies) e no ladrilho de servidor do rail. ⚠ **`membro.avatar` e não `membro.avatarURL`, e isso me pegou:** o getter do SDK já encadeia `this.avatar?.createFileURL() ?? this.user?.avatarURL`, e o do usuário, sem anexo, devolve a URL do avatar PADRÃO gerado pelo servidor — uma silhueta igual para todo mundo, que cobriria o gradiente. Medido antes: **41 `<img>` para `/default_avatar`** numa tela onde ninguém tem foto. Depois: 0. ⚠ O ponto de presença vem DEPOIS da imagem no DOM, sem `z-index`: quem pinta por último fica por cima. A primeira versão perdeu o `{children}` inteiro, e o lint de variável não usada foi quem acusou. |
| `client` não tinha como subir | **Dockerfile e workflow escritos, com o mesmo truque da referência `vendor/stoat-web/`:** estágio de build pinado em `$BUILDPLATFORM`, porque o artefato é JS e roda nativo no runner amd64 mesmo com a imagem sendo arm64 — sem isso seria uma hora sob QEMU. ⚠ **Sem o `inject.js` do upstream, e é a simplificação que o `location.origin` paga:** o cliente Solid constrói com `VITE_API_URL=__VITE_API_URL__` e reescreve os placeholders no bundle quando o contêiner sobe. Aqui a URL acompanha o domínio sozinha, então a etapa de reescrita — que pode falhar em silêncio — deixa de existir. As `VITE_*` do compose ficam inertes para esta imagem, e o resumo do workflow diz isso. |
| Back-end local já existe | **`make vortex-local-env` e `make vortex-local-up` no `pi-infra` sobem a stack inteira em `http://localhost:8880`** — API, events, autumn, january, LiveKit, Mongo, Valkey, RabbitMQ, MinIO e Caddy, com os hosts reescritos para HTTP sem TLS. É o que falta para VERIFICAR o caminho de rede: hoje o `login()`, o `connect()`, o `Ready`, o envio e o upload nunca rodaram contra um servidor. Precisa do Docker Desktop no ar; a CLI está instalada e o daemon não estava. |
| ⚠ A coluna de conteúdo contava FILHOS | **O defeito que quem usa relatou como "o chat está cortando e o input está no topo da página", e ele estava lá desde que o cabeçalho virou linha do shell.** `.conteudo` era `grid-template-rows: auto auto minmax(0,1fr)` — barra do arnês · cabeçalho · prancha. Três trilhas para três filhos, e **dois deles são opcionais**. No arnês fecha; no produto não há barra de firehose, então o cabeçalho toma a trilha 1, a prancha cai na trilha 2 (`auto`, altura de conteúdo) e a trilha de `1fr` fica VAZIA no fim da coluna. Medido em 1142px: trilhas `50px 360,4px 731,2px`, prancha em 360 e **731px de nada embaixo**. Virou coluna FLEX, que não conta: toda barra é `flex: none` e a prancha é a única que cresce, com zero, uma ou duas barras acima. Medido depois: prancha 50..1142, timeline 986, `textarea` em **1075** (era 344). ⚠ **Nenhuma guarda pega isto** — a regra é válida, o token é válido, o contraste é válido, e a única coisa errada é a CONTAGEM de filhos, que o CSS não vê. É a enésima vez que o arnês esconde um defeito por ser mais rico que o produto, e a segunda em que ele o esconde por RENDERIZAR A MAIS em vez de a menos. |
| ⚠ A escala de ícone existia DUAS vezes | **Achado medindo a tela depois de aplicar a minha, e eu quase subi uma segunda escala sem saber.** `tokens.css` define `--vx-icon-0..3` (12·14·16·20) desde antes, com **102 consumidores** e a razão escrita; o `ICONE` do ponto único nasceu ao lado, com os MESMOS quatro números e nomes diferentes. Não colidiram porque um vive em CSS e o outro em TSX — e nada os ligava, então mexer num não quebra o outro e o sintoma seria um ícone de 14 ao lado de um de 16 na mesma barra. Amarrados por `pareamento` + teste que lê o `tokens.css` de verdade, nos dois sentidos, verificado por **cinco mutações**. ⚠ Os nomes seguem semânticos aqui e ordinais lá de propósito: dentro de uma regra de CSS o componente já se identifica, num `size={}` solto `--vx-icon-1` não diz nada e `metadado` diz. |
| ⚠ 43 props de tamanho de ícone eram MORTAS | **Resolvidas — zero na tela, medido em seis superfícies.** Elas traziam `size={20}` no TSX e desenhavam 9, 12 ou 14, porque uma regra como `.acoes svg { inline-size: var(--vx-icon-1) }` vence a prop. Os valores RENDERIZADOS estavam quase todos certos; o que estava errado é o TSX mentir — a família do comentário que afirma uma medida que não existe. O dono certo é o CSS: são **42 regras** de container contra um punhado de `size={}`, e o padrão é deliberado (acrescentar um ícone a uma barra já sai no tamanho certo). Quem sobrava era a prop. ⚠ **Estático não fechou, e a tentativa está registrada porque a lição é sobre INSTRUMENTO:** um analisador de JSX que escrevi achou 27 sítios e perdeu o resto — a regra vencedora mora num arquivo e o ícone em outro, e `<Tooltip>` com `onClick={() => …}` tem chaves aninhadas que quebram qualquer tokenizador que não seja um parser. O navegador já sabe a resposta. |
| ⚠ As props mortas VOLTARAM, e nenhuma tinha sido escrita por ninguém | **87 instâncias, sob 12 regras de CSS, todas relatando `prop 24` — e `24` é o padrão do PACOTE.** `@remixicon/react` declara `size = 24` e o estampa como `width`/`height` no `svg`, então um `<Smiley />` sem prop nenhuma chega ao DOM afirmando 24px enquanto `.sm svg { inline-size: 13px }` desenha 13. ⚠ **Eu procurei o defeito nos call sites e ele não estava lá:** os sítios reais dizem 18, 16 ou 14, e que os 87 dissessem o MESMO número era a assinatura de origem única — a barra de ações da `MessageRow`, que eu tinha como suspeita, passa `<Smiley aria-hidden />` sem `size`. O conserto é uma linha no wrapper (`size={size ?? "1em"}`), e cobre os 87 porque o lint de boundary já impede importar `@remixicon/*` fora de `components/ui/icones.tsx`. Medido depois: **182 ícones em quatro superfícies (shell · configurações · paleta · voz), 0 com dois donos, 0 sem dono, 0 fora da escala**. |
| ⚠ A guarda seguia verde com um ramo INALCANÇÁVEL | **A metade pior do item acima, e ela é sobre a guarda, não sobre ícone.** `dev/tamanhoDeIcone.ts` decide se a prop mente lendo o atributo `width` e trata `1em` como "ninguém pediu tamanho" — o sentinela que o `IconContext.Provider` do Phosphor produzia. O PR #205 **apagou o `PesoDeIcone.tsx`** junto com a troca de biblioteca, e o pacote passou a estampar o próprio padrão: com um número em todo `svg`, `temProp` virou sempre verdadeiro e o ramo **"nenhum dono" ficou impossível de disparar**. A assertion rodava, não acusava nada, e não podia mais. É "medir com o instrumento desligado" pela 6ª vez — a primeira em que o instrumento desligado é uma GUARDA e não o ambiente. ⚠ **O que a assertion não podia se defender sozinha:** o sentinela dela era detalhe de implementação de uma dependência, e trocar a dependência o removeu sem tocar no arquivo dela. Agora o contrato é TESTE (`icones.test.ts`), que fica acima de assertion na ordem do enforcement e não precisa de navegador: renderiza os 89 ícones exportados por `renderToStaticMarkup` e exige `width="1em"` sem `size`, mais um caso provando que quem PEDE tamanho continua obedecido. Verificado por mutação — tirar o `?? "1em"` reprova 89 dos 132 testes do arquivo, mostrando o `width="24"` do pacote. Uma troca de biblioteca de ícone agora quebra o `pnpm check` antes de qualquer tela. |
| ⚠ A prova de que o dono é único virou assertion | `dev/tamanhoDeIcone.ts`, e ela é **simétrica**: dois donos (a prop mente) e nenhum dono (o `svg` sai no `1em` herdado, que é o oposto de uma escala) são os dois defeito. Achou de primeira um caso que o analisador estático não via, e é ela que fecha o que sobra nos arquivos que outra sessão está editando — edição perdida se denuncia sozinha. Mora no `Cliente.tsx` e não numa superfície, porque varre o documento: modal, painel e configurações montam fora daquela árvore, e é justamente o que monta depois que estático não alcança. Some do bundle de produção, inclusive do que o `pnpm gate` mede. Medido depois: **129 ícones, 0 com dois donos, 0 sem dono, 0 fora da escala**, em tela principal · paleta · três seções de configurações · chamada · transmissão · grade. |
| O caret de 9px NÃO era defeito, e eu afirmei que era | ⚠ **Minha pendência anterior dizia "fora de escala, e o design não decide". O CSS tinha a razão escrita duas linhas acima do valor**, medida: é triângulo de DIVULGAÇÃO ao lado de um rótulo de 11px em caixa alta, e em 14 ficava maior que o texto que anota. A referência usa 12 no mesmo lugar — e a regra de método do projeto resolve o empate: a referência decide o que EXISTE, o design decide os VALORES. Ficou em 9. Mesma coisa no `13px` do `BotaoDeIcone` ("13 dentro de 28 é o do design"). Os dois agora estão em `FORA_DA_ESCALA`, com a razão e um teste nas duas direções: valor fora da escala sem entrada reprova, e entrada que voltou para a escala também. O `11px` do `Marcador` era o único SEM razão escrita, e virou degrau. |
| ⚠ O conjunto que parece coeso usa DOIS pesos | **A pergunta era "por que os ícones do Discord parecem melhores", e a resposta não estava em nenhum desenho.** Medido rasterizando as formas num canvas a 13px — onde vivem 65 dos 129 ícones da tela: o `regular` que estava no ar depositava 14,6% de tinta e só **3,8% chegava opaca**; o anel inteiro da lupa tinha QUATRO pixels opacos e todo o resto era cinza de anti-serrilhado. O ícone não estava desenhado, estava sugerido. `fill` entrega 29,9% opaca e virou o padrão. ⚠ **Mas `fill` não faz uma coisa só:** em uns ele solidifica a MESMA forma (sino, cadeado, microfone — a silhueta já era o ícone), em outros desenha OUTRA — o glifo vazado dentro de um quadrado. `X`, `Plus` e `Check`, os três mais usados do app, caem no segundo grupo. O critério é a IoU da SILHUETA entre `fill` e `regular` a 48px, com corte em 0,80 e um vão claro nos dados entre 0,45 e 0,57: `DotsThree` 0,07 · `Check` 0,13 · `Plus` 0,20 · `X` 0,21 · `Hash` 0,42 contra `SpeakerSlash` 0,89. **64 sólidos, 27 de contorno.** A referência confirma a regra: o `guildCross` do Discord é um X de TRAÇO dentro de um círculo (o círculo é crachá semântico), e o `guildCheck` é um ✓ de traço grosso sem caixa. |
| Duas exceções de ícone são JULGAMENTO, e vão marcadas | ⚠ `MagnifyingGlass` e `Square` **passam** no teste de silhueta — preenchida, a lupa ainda é círculo com cabo. O que elas perdem é uma ABERTURA, e abertura é semântica: a lente quer dizer "você vê através", e o quadrado é o contorno de uma JANELA no botão de maximizar da barra de título. Tentei medir e a métrica super-selecionou: ela acusava `BellSimple`, `Microphone` e `VideoCamera` junto — os que ficam ótimos sólidos, porque o miolo deles nunca foi a identidade. Não há régua; há uma pergunta: **o vão quer dizer alguma coisa?** A distinção fica escrita porque exceção medida e exceção julgada envelhecem diferente. |
| ⚠ Li o segundo eixo ao contrário e quase publiquei o inverso | **Contei "vão interno sobrevivente" e escrevi que a lente FECHAVA no `regular`** — o palpite razoável, já que engrossar preenche vão. O mapa de alfa desmentiu: o vão está lá, quem não está é o ANEL; meu contador devolvia zero porque o vazio VAZAVA pelas falhas do traço. A correção inverte de quem é o risco, e o erro só apareceu porque eu desenhei a imagem antes de publicar o número. Vale como método: **número de topologia precisa da figura ao lado**, senão ele diz o contrário do que parece dizer. |
| Gate depois dos ícones: SEM veredito | ⚠ **Duas corridas, as duas INVÁLIDAS** — 428/500 (86%) e 416/500 (83%), abaixo do piso de 90%. A máquina tinha Radeon Software, Discord, Gather e Spotify no ar, que é exatamente o perfil de 83–87% que esta tabela já registra. **Não é regressão nem aprovação**, e a guarda de vazão é que impediu o relatório falso. O que precisa de medição: os 27 envoltórios de `deContorno` acrescentam uma camada de componente por ícone, e as formas de `fill` são maiores para rasterizar. `pnpm check` passa inteiro (502 testes, 59 arquivos). Refazer mediana de 3 com a máquina limpa. |
| Tri-state: "acima da sua hierarquia" | **CONSTRUÍDO no menu de membro — esta linha dizia "fora", e envelheceu.** Verificado em navegador: o item `Moderar · acima da sua hierarquia` renderiza desabilitado quando o alvo está acima na hierarquia, e o comentário do código explica a regra que o sustenta — o aviso e os três itens de moderação são EXCLUDENTES, porque a primeira versão os deixou coexistir e a interface se contradizia numa linha. ⚠ **O que continua fora é o tri-state de PERMISSÃO** (`Administrador · Bloqueado — acima da sua hierarquia`, no editor de cargos), que é outra superfície e outro sentido da palavra — confundi os dois uma vez nesta varredura. |
| ⚠ O repositório foi REESTRUTURADO, e meu inventário envelheceu 111 commits | **`web-react/packages/client/` virou `client/packages/client/`**, e a `main` deste repo é o layout upstream. Uma sessão trocou de branch pelo GitHub Desktop no meio do trabalho, o stash automático guardou tudo, e o cliente React inteiro sumiu do disco — porque `main` não rastreava `web-react/`. ⚠ **A lição não é sobre git, é sobre MEDIÇÃO:** a varredura de features que eu tinha acabado de entregar foi feita numa árvore 111 commits atrás, e errou em pelo menos cinco pontos — modo compacto, `SidePanels`, `Poll`, `PilulasDeCargo` e o menu de usuário estavam todos feitos. Inventário tem data de validade, e ele não a carrega escrita. |
| Varredura 1:1 contra os DOIS oráculos — o método | **A referência decide o que EXISTE, o design decide os VALORES, e conferir só a primeira leva a veredito errado.** Duas vezes nesta varredura eu classifiquei algo como "invenção da referência, não queremos" e o design o desenhava: `GameOverlay` e `ActivitiesModal` estão numa seção própria dele (`POPOUT / OVERLAY / ATIVIDADES`), com atalhos definidos (`Atender ⌘↵`, `Recusar esc`). E `Bloquear`, que eu ia pôr no menu de membro, o design põe no menu de **DM**. Presença de arquivo também não basta: sete componentes do lote 4 não tinham arquivo com o nome e existiam por conteúdo. |
| ⚠ Quatro erros de instrumento numa varredura só | **Todos com a mesma forma: a medida frouxa concorda com a hipótese.** (1) Medi `.calha` em vez de `.horaCompacta` e quase publiquei que a coluna de hora do compacto não era mono — ela é, em JetBrains Mono a 11px. (2) Filtrei `--include=*.tsx` e o `store/silencio.ts` ficou fora: reportei três durações de silenciar quando existem as cinco. (3) Casei `:active` por seletor sem olhar descendente, e o press do rail (que pousa no ladrilho filho) contou como ausente. (4) Expandi `background` em longhands e comparei com a shorthand da `transition-property`, inflando "hover seco" de 26% para 74%. A regra: **quando o número confirma o que você esperava, meça de novo com outro instrumento.** |
| `pnpm sombra` — declaração viva, regra morta | **Guarda nova, e ela nasceu de um defeito MEU.** Acrescentei `.membro:active` à member list sem ver que o arquivo já tinha um cem linhas abaixo; mesma especificidade, a última ganha, a minha não produzia efeito. Nenhuma das nove guardas via: `pnpm utilities` confere `className` sem regra e regra sem `className`, e a classe TEM consumidor — o que morreu foi a DECLARAÇÃO, dentro do bloco. Confere mesmo seletor, mesmo contexto de at-rule, propriedade repetida; `@media`/`@container` são isentos porque ali a segunda é condicional. Verificada por mutação e três controles. **Achou duas sombras pré-existentes na primeira corrida** (`Rail .marcaCasa` e `AdicionarServidor .sobrancelha`), e as mesmas duas seguiam intactas em `main` 111 commits depois. |
| `pnpm movimento` — a doutrina do Foundations vira mecanismo | **O design escreve a regra inteira numa linha** — *"Só `transform` e `opacity`. Nada anima acima de 240ms; listas nunca animam altura"* — e o `tokens.css` a repetia em prosa. ⚠ **A leitura literal seria errada, e o próprio design a desmente:** ele tem 125 transições de `background`, `border-color` e `color`. O que a frase proíbe é o que MEXE A CAIXA, e é isso que a guarda confere. Fora de fluxo (`position: absolute`) é DERIVADO e não enumerado — as exceções caíram de quatro escritas à mão para uma. ⚠ **Ela passou de primeira e estava quebrada:** varria por início de linha, e 3 de 4 mutações passaram por ela. Reescrita sobre o texto, passou a ver 112 declarações onde via 58 — metade. |
| A mesma pessoa tinha dois menus | **Achado do lote 2, e o `CLAUDE.md` já tinha a regra escrita desde o dia do `⋯`:** *"Dois menus com os mesmos quinze itens divergem no primeiro que ganha um item novo."* Timeline: onze itens. Member list: quatro, só moderação. Já tinham divergido em sete. `ItensDoUsuario` saiu de `list/MessageRow.tsx` e virou `membros/MenuDoUsuario`, porque é menu sobre uma PESSOA e todas as dependências dele já viviam ali. ⚠ **O `podeModerar` que gateava o menu protegia o CUSTO, não a permissão** — Root por linha em dezenas de milhares. Com o Root no nível da lista e o alvo no store, a proteção deixou de ser necessária e o custo CAIU. ⚠ O menu unificado é a UNIÃO: "Copiar ID" existia só na member list, e trocar sem trazê-lo teria apagado a ação em silêncio. |
| O que a varredura achou e AINDA não foi feito | **Erro de implementação, barato:** `BotaoDeIcone` sem tom `aviso` (a família existe em `Botao`, `Banner` e `Selo`); menu de DM — o design pede cinco itens e `bloquear`/`desfazerAmizade` **já funcionam** em `casa/Amigos.tsx`, e o argumento de custo contra ele (Root por linha) morreu com o padrão do parágrafo acima; `Colapsar todas`; `Duplicar canal`. **Divergência 1:1 medida:** o seletor de qualidade da transmissão tem UM eixo e o design tem dois (`Resolução` 720/1080/**1440** × `Taxa de quadros`) — sem 1440p, e 720p a 60 fps é incombinável; entrelinha do compacto em 1,50 contra 1,45 do design, que é `leading-relaxed` do Tailwind sem degrau equivalente na escala. **Sem consumidor:** `Tabs`, `Textarea`. |
| A superfície ausente de maior valor: registro de auditoria | **É a única que falta com o protocolo A FAVOR.** `GET /servers/{target}/audit_logs` existe, com filtro por autor, alvo e tipo, e `AuditLogEntryAction` é união marcada. Ela destrava de quebra as duas colunas de Banimentos que hoje não existem ("Banido por" e "Data"), e a palavra "auditoria" já aparece em treze telas do produto prometendo um lugar que não existe. Fica FORA do `pendencias.ts` pelo contrato dele — não há controle para clicar. |
| ⚠ Nada da varredura entrou no `pendencias.ts`, e isso é a guarda funcionando | **O contrato dele é estreito de propósito:** *"controles DESENHADOS que ainda não fazem nada"*, com toast dizendo o que farão. Os achados desta varredura são AUSÊNCIAS (não há controle na tela) ou DEFEITOS — nenhum se qualifica. Forçá-los ali transformaria o registro no depósito que a própria doutrina do arquivo avisa contra, e faria as 52 entradas honestas valerem menos. As cinco de voz, os eventos, o fórum, a mídia, os tópicos e a bandeja **já estavam lá**, cada uma com a dependência exata escrita — o registro estava fazendo o trabalho dele antes de a varredura começar. |
| ⚠ Toda linha reservava espaço de avatar, inclusive sem avatar | **Relatado por quem usa — "o tamanho fica disparelho do conteúdo real da mensagem" — e a causa é literalmente essa.** `.calha` renderiza em TODA linha com `block-size: 40px` fixo, mas o conteúdo dela é condicional: avatar ao abrir grupo, hora no compacto, e **`null` na continuação**. Sobrava uma caixa vazia de 40px segurando uma linha cujo texto é uma frase de 22px. Medido no navegador: mensagem curta agrupada saía a **50px onde precisa de 30** — 20px de sobra morta, **67% de inflação**. A altura saiu da calha; quem tem 40px é o AVATAR, e ele já os carrega sozinho. ⚠ **Nenhuma guarda podia pegar, e a razão é o INSTRUMENTO:** o firehose gera texto longo, então com três linhas de conteúdo 40px nunca é o maior dos dois — a calha nunca segura nada e a média que alimenta `ALTURA_POR_TIPO` mal se move. É a 11ª ocorrência de "arnês mais pobre que o produto", e a primeira em que o que falta ao arnês não é um CAMPO, é o tamanho típico do dado. |
| ⚠ A primeira versão do conserto da calha esticava o alvo do menu | **Tirar só o `block-size` deixa o `stretch` do flex esticar a calha até a altura da linha: medido, de 40 para 151px.** E ela carrega o `data-menu-autor` — esticada, clicar com o direito em qualquer ponto da coluna esquerda abriria o menu da PESSOA em vez do da MENSAGEM, ao longo de parágrafos inteiros. `align-self: flex-start` devolve a caixa ao tamanho do avatar. Verificado depois: **40×40 com avatar, 40×0 na agrupada** — inclicável ali, que é o certo. A lição é a de sempre neste arquivo: o conserto óbvio de layout mexe em ALVO sem avisar, e só a medição do alvo mostra. |
| Firehose: quatro lotes sem veredito | ⚠ **Toquei a `MessageRow` três vezes nesta rodada e não tenho medição para nenhuma.** Seis corridas do gate: 71%, 73%, 87%, 88%, 89% e 89% de vazão contra o piso de 90% — todas INVÁLIDAS, que não é PASS nem FAIL. O perfil bate com o que esta tabela já registra para a máquina com Discord, Gather e Trae no ar, e as duas piores foram com o preview e o navegador disputando CPU junto. O que dá para dizer sem medir: o passe de movimento é CSS que só roda na linha sob o ponteiro, uma por vez; e a unificação dos menus REMOVE montagem por linha. Expectativa não é medição, e fica registrado como dívida. |
| O stash do GitHub Desktop guardava trabalho que nenhuma branch tinha | **Resgatado, e ele era invisível por CAMINHO.** O auto-stash de 04/09 estava no layout ANTIGO (`web-react/`), então nenhuma comparação contra a `main` o via — não há arquivo com o mesmo nome dos dois lados. Promovido a commit em `resgate/stash-do-github-desktop` (preservação, não proposta: um PR dali proporia apagar `client/` e ressuscitar `web-react/`). Dos quatro itens que ele tinha a mais, **dois já haviam chegado por outro caminho** — a fusão dos dois blocos `.marcaCasa` e os cinco `:active` — e os que sobraram viraram commit próprio. ⚠ Um stash é ref frágil: `git stash drop` o apaga sem confirmar, e nada avisa. |
| ⚠ O comentário de `--vx-revelar-*` prometia um consumidor que não existia | **A família do `py-0.5`, agora em documentação de token.** O bloco em `tokens.css` nomeia DUAS superfícies servidas pelo par — a barra de ações da mensagem e o `+` de categoria — e só a primeira o consumia; o `+` tinha `visibility: hidden` puro, que não interpola, e pipocava. Nenhuma guarda vê isso: o token existe, o consumidor existe, o contraste passa, e o que está errado é uma AFIRMAÇÃO em prosa sobre quem usa o quê. Mesma forma do comentário de `.corpo` que afirmava uma medida de leitura que o parágrafo não tinha. |
| Quatro hovers trocavam cor sem tempo | **Fechados, e os quatro eram vizinhos de alvos que já acompanhavam.** `.membro` da member list (o fundo tinha transição, a cor não — e o hover troca as duas), `.marcaAdicionar` e `.marcaRodape` do rail (a `.marca` dos servidores a 8px de distância já tinha tempo) e `.servidor` no cabeçalho da coluna de canais. O `pnpm movimento` não os pega de propósito: ele guarda o que MEXE A CAIXA, e cor não mexe. O que denuncia estes é o vizinho — dois ladrilhos na mesma coluna, um com tempo e um sem. |
| Gradiente de cargo — LIBERADO, e o protocolo sempre o aceitou | **Três lugares deste repositório afirmavam que não dava, e os três estavam errados.** `pendencias.ts` dizia "o protocolo tem só colour, ou seja fork do serviço api", `tema/cargo.ts` e `Cargos.tsx` repetiam. O `RE_COLOUR` de `server_members.rs` aceita `(repeating-)?(linear\|conic\|radial)-gradient(...)` em `colour` até 128 caracteres, e o cliente Solid de referência já o desenhava (`ColouredText.tsx`). O estilo não é GUARDADO: é LIDO da forma de `colour`, então abrir um cargo salvo em gradiente abre com "Gradiente" marcado sem campo nenhum a mais. ⚠ **O que travava era a NOSSA garantia de contraste:** gradiente não passa por `hexParaOklch`, e o `catch` devolvia ausência. Agora cada parada passa pelo clamp de L, e a interpolação é **`in oklab`** — em sRGB o meio de duas paradas de mesmo L fica mais escuro que as duas, e o contraste medido nas paradas não valeria entre elas. Medido: tema claro, paradas a **8,03:1 e 8,91:1** sobre a lista. Pareamento com o servidor por teste que LÊ a regex do `.rs` do disco; quatro mutações verificadas. |
| ⚠ A string de `colour` nunca vai ao DOM | **Segurança, não gosto.** `colour` é escrito por quem administra QUALQUER servidor onde a pessoa esteja, e o `RE_COLOUR` aceita `var(--…)` — que leria os tokens deste app. `lerGradiente` aceita só `linear-gradient` com paradas hex e posição em `%`, recusa qualquer parêntese no corpo (`rgb()`, `var()`, `url()`), e o CSS que chega ao `style` é RECONSTRUÍDO de números e hex já passados pelo clamp. Cônico e radial degradam para a primeira parada. Onde o gradiente não entra — autor na timeline, cartão de perfil — `corDeCargo` devolve a primeira parada, e não mais a ausência. |
| O holográfico do design NÃO é animado, e eu tinha dito que era | ⚠ **Afirmei "iridescência animada" sem abrir o design.** O design o escreve como gradiente ESTÁTICO (`linear-gradient(100deg,#8FE9F0,#C9B6F5 45%,#F3C6A8)`), que o `RE_COLOUR` também aceita — então ele não depende de fork nenhum. A pendência virou `estiloHolografico` com `depende: só a tela`: reconhecer o preset na leitura e a pílula clara de texto escuro que o design pede. É o erro de sempre desta tabela com a forma de sempre: a conclusão confortável ("precisa de backend") aceita sem a medição que a desmentiria. |
| Holográfico — LIBERADO, e com uma regra de contraste diferente da do gradiente | **Mesmo mecanismo do gradiente, paradas fixas:** o editor grava `HOLOGRAFICO` (`linear-gradient(100deg, #8FE9F0, #C9B6F5 45%, #F3C6A8)`), que passa no `RE_COLOUR`, e a leitura o RECONHECE por igualdade das paradas — não do texto, porque o mesmo preset gravado por outro cliente com outra caixa ou espaço é o mesmo holográfico. A pílula é a do design: preset em opacidade cheia e texto `#101318` (`TINTA_HOLOGRAFICA`), um par que não muda com o tema porque o fundo não muda. ⚠ **O nome NÃO passa pelo clamp de L, e é decisão medida.** O clamp faz duas coisas — garante 4,5:1 e puxa todo cargo para o L de `text-2` para não competir com quem não tem cargo —, e a segunda metade apagaria o preset: os pastéis vivem em L ≈ 0,85–0,90. Então cada parada fica CRUA onde já passa 4,5:1 nas quatro superfícies do modo, e só vai para o clamp onde não passa. A regra é derivada, não enumerada: no escuro as três passam e o nome sai **byte a byte do design**; no claro nenhuma passa e as três são clampadas, medidas em **8,01 · 8,63 · 8,49:1** sobre a lista. Quatro mutações verificadas (sempre clampar, nunca clampar, tinta clara na pílula, reconhecimento sem posição). A pendência `estiloHolografico` saiu do registro. |
| Nome silenciado não recua quando o cargo tem cor | **Pré-existente, achado lendo a regra durante o gradiente.** `.membro[data-silenciado="true"] .nome { color: text-3 }` perde para a cor de cargo, que entra por `style` inline — e inline vence qualquer seletor. Quem foi silenciado "só para mim" continua com o nome colorido, que é o contrário do que o recuo existe para dizer. Vale igual para o gradiente. Não corrigido: fora do escopo da troca de estilo. |
| Gate depois do gradiente: NÃO rodado, e a razão é medida | `NomeDoAutor` passa por `corDeCargo`, que agora consulta um cache por (modo, string) em vez de refazer duas conversões OKLCH a cada render — mais barato, mas expectativa não é medição. Não rodei porque a máquina tinha `deadlock.exe`, Discord, Gather e Spotify abertos, que é o perfil que esta tabela já registra como corrida INVÁLIDA (83–87% de vazão). Correr para produzir um não-veredito seria o instrumento desligado de novo. |
| Silêncio persistido e sincronizado | **Resolvido — F5 zerava silêncio, nível por canal/servidor e opções de supressão.** Duas chaves de `UserSettings`: `notifications`, no formato do cliente OFICIAL (quem abre a conta no Stoat vê os mesmos silêncios), e `vortex:notificacoesDoServidor` para `suprimirTodos`/`suprimirCargos`, que o oficial descartaria no `clean()`. ⚠ "Até eu reativar" é mute SEM `until` — `JSON.stringify(Infinity)` vira `null`. ⚠ **Hidratar troca os mapas crus, nunca passa pelos setters:** o acoplamento "nada ⇒ silencia" reescreveria um `none` sem mute vindo do cliente oficial num silêncio que ninguém pôs. Verificado em navegador: silenciar `#geral`, recarregar, e ele volta silenciado. |
| Privacidade por servidor com efeito | **Filtro de mídia e DM no cliente, DM e amizade no `delta`, presença pendente.** A mídia soma o filtro pessoal à política do servidor (nunca subtrai); DM de quem só está ligado a você por servidores que negam vai para a fila de solicitações; `open_dm`, `send_friend_request` e `add_friend` (que também CRIA pedido) leem a chave do destinatário e recusam com `PrivacyRestricted`, só quando TODOS os servidores do vínculo negam e não há amizade, pedido recebido, DM ou grupo. ⚠ **Padrão CONSERVADOR, divergindo do design** (que marca "cargo em comum" e "filtrar de não-amigos"): com efeito real, aquele padrão fecharia DMs e velaria mídia de quem nunca abriu o modal. ⚠ **Formato ganhou `versao: 2`** porque todo cliente já tinha subido o padrão de fábrica antigo — sem versão, o servidor ignora e o cliente migra. "Atividade em jogo" saiu do modal (o protocolo não a tem); "aparecer offline aqui" é `presencaPorServidor` no registro de pendências. **Não verificado em navegador: a fila de DM** — o arnês não serve `/users/{id}/mutual`. Faltam o aviso no lugar do composer (D-LAC-33) e "aparecer na lista de membros" (D-LAC-31). |
| Testes do `revolt-delta` não compilavam na main | **Consertado em commit próprio.** `rand` 0.10 removeu `thread_rng`/`distributions` (`util/test.rs`) e dois `PartialMember` de `message_send.rs` não tinham `show_tag`. Rodando em container (`rust:1.98`, rede do compose `vortex-teste`), os testes de rota precisam de `TEST_DB=MONGODB` e `mongodb://database/?directConnection=true` — o Mongo é replica set anunciando `127.0.0.1`. ⚠ Seguem falhando os de `account::*` que leem e-mail em `http://localhost:14080` fixo: ambiente, não código. E a `ReferenceDb` tem `fetch_mutual_*` como `todo!()` e `set_user_settings` estendendo o mapa consigo mesmo. |
| Gate depois da privacidade e do silêncio: INVÁLIDO | `MidiaFiltrada` ganhou duas assinaturas (privacidade por valor e `useEhAmigo`, booleano para a presença do autor não acordar a mídia). Seis corridas, todas INVÁLIDAS a 79–84% de vazão, com `deadlock.exe`, Radeon Software, Spotify e Discord no ar e outros agentes compilando. Não é PASS nem FAIL. |
| Atualização obrigatória do desktop | **Alcançável agora, e era construída e inalcançável desde a etapa 9.** A casca nunca emitia `obrigatoria`, "Atualizar e reiniciar" só agia em `pronta` e "baixar manualmente" abria a PASTA DE LOGS. Agora o `delta` expõe `features.desktop_min_version` (opcional, ausente por padrão) e o CLIENTE decide o bloqueio comparando com `ponte().versao` — quem sabe a versão mínima é o servidor, e a casca não fala com ele. `vortexInstalarEReiniciar` aceita `{ obrigatoria }` e instala mesmo sem nada baixado (verifica, baixa, instala no `update-downloaded`); a decisão mora em `atualizacaoModelo.ts`, puro e testado. ⚠ **Argumento opcional e não verbo novo:** verbo em `PonteDesktop` faria toda casca anterior contar como ausente; o preload antigo descarta o argumento, e o store desiste após 10 s sem resposta e mostra "Tentar de novo" + download manual. ⚠ **O bloqueio TRAVA** — medido no arnês antes da trava: o primeiro clique levava a casca a `verificando` e o app destravava numa versão que o servidor recusa. `falhou` virou faixa (regra do design, D-CASCA-24) e `verificando`/`baixando` aparecem em Configurações > Desktop. **Fora:** Cancelar, Pausar, "Depois", barra de bytes (o Squirrel não reporta — progresso é 0 ou 100), canais e notas da versão (D-CASCA-22/26/27). |
| ⚠ Clique direito: o mecanismo era por superfície, e cobria UM gesto de três | **Seis superfícies escritas em seis momentos, cada uma resolvendo "quem é o alvo" do seu jeito — e só a timeline tinha mais de um caminho de abertura.** O pior era silencioso: o long-press do Radix (700 ms, toque e caneta) abre o menu **sem disparar `contextmenu`**, e todas as superfícies escutavam só `contextmenu`. No toque o menu vinha com o alvo do gesto ANTERIOR — "Excluir" na mensagem errada. Agora há um gatilho único (`components/ui/MenuDeContexto.tsx`) que mira no `pointerdown`, no `contextmenu` e na tecla, com `preventDefault` quando não há alvo. **Medido no arnês, 10 mil mensagens:** toque longo na linha B depois de um clique direito na linha A deixa o alvo em B; toque no vão e na linha de SISTEMA não abre nada; `Shift+F10` e a tecla Menu na linha focada abrem os 15 itens; com foco no container, nada. ⚠ **A resolução saiu do componente e virou função pura do DOM** (`alvoNoDom`): a linha de mensagem e a de membro perderam um `onContextMenu` por linha e ganharam um atributo estático — menos trabalho por linha nos dois componentes virtualizados. ⚠ **`modal` fica no default, e por medição:** abrir e fechar o menu não move `scrollTop` (991663 nos três momentos), nem o topo da primeira linha (−658), nem o `padding-right` do `body` (0px, porque o `body` deste app não rola). |
| ⚠ Toque longo abria DOIS menus na coluna de canais | **Com mouse não acontece, e é por isso que ninguém tinha visto.** O gatilho interno chama `preventDefault` no `contextmenu` e o externo o vê prevenido na bolha; o long-press não passa por ali — cada gatilho arma o próprio timer de 700 ms no `pointerdown` e os dois abrem. A linha de canal abria o menu dela E o da área vazia. O gesto agora é REIVINDICADO por quem o pega primeiro (o mais interno, porque o evento sobe), e o de fora chama `preventDefault` para o Radix nem armar. Medido: toque longo numa linha de canal devolve **uma** caixa de 8 itens. |
| Paleta ⌘K executava no clique DIREITO e no do meio | **`onMouseDown` dispara para os três botões; `click` só para o principal** — trocar um pelo outro (por causa do foco que o Dialog perde no blur) trouxe junto dois gatilhos que ninguém quis. Clicar com o direito num resultado NAVEGAVA para ele, na superfície cujo trabalho inteiro é "ir para". `if (e.button !== 0) return`. |
| `pode("")` escondia a administração de quem administra | **Categoria VAZIA e servidor SEM CANAL caíam no `pode("")`, que é `false` com sessão presente.** O comentário do código já descrevia o beco e terminava com "enquanto o SDK só responde por canal" — ele responde por servidor também. `podeNoServidor` existe **só** para onde não há canal a que perguntar: perguntar ao servidor onde existe canal devolveria a resposta errada, porque sobrescrita por canal só aparece no canal. |
| Clique direito morto na voz e na raiz do tópico | **Três superfícies não marcavam de quem era o ladrilho**, então o `ComMenuDoParticipante` que envolve o palco recusava abrir: a tela de quem transmite, a chamada direta e o PiP próprio. A raiz do tópico era pior — ela é um `<article>`, então o menu ABRIA, vazio, porque nenhuma linha tinha escrito alvo. Na chamada direta a marca vai na SEÇÃO e não no avatar: com câmera ligada o vídeo cobre a tela como irmão do avatar, e marcar só o avatar deixaria morto justamente onde há o que olhar. |
| Barra de ações da mensagem no toque | **`:hover` num dispositivo de toque GRUDA** — o navegador o emula no último elemento tocado e não o desfaz, então tocar uma mensagem para rolar deixava oito alvos flutuando sobre o texto até alguém tocar noutra linha. Suprimida em `(hover: none) and (pointer: coarse)`; `:focus-within` e o menu aberto continuam revelando, então teclado físico num tablet não perde a barra. Ela sempre foi afordância de PONTEIRO — o caminho do toque é o toque longo, e ele agora funciona. |
| ⚠ A mesma ENTIDADE tinha tres menus, e nenhum com item em comum | **O servidor era o caso extremo, e ele estava na cara:** o clique direito no ladrilho do rail dava privacidade e pastas; o `▾` do cabecalho da coluna dava criar canal e as TREZE secoes de configuracao soltas; o clique direito no proprio cabecalho caia no menu do navegador. Zero intersecao. Agora `menus/ItensDoServidor.tsx` serve aos tres — medido em navegador: **13 itens identicos** pelo rail, pelo `▾` e pelo clique direito no cabecalho. Mesma historia no canal (`ItensDoCanal`, coluna e `⋯` novo do cabecalho: 9 itens identicos), na conversa (`ItensDaConversa`) e na pessoa. ⚠ **`ContextMenu` e nao `DropdownMenu` mesmo nos botoes:** o `▾` e o `⋯` despacham o `contextmenu` que o gatilho ja escuta, o mesmo arranjo do `⋯` da barra de acoes. Com as duas familias de primitivo o conteudo teria de ser escrito duas vezes — e e exatamente assim que os tres divergiram. |
| O menu de VOZ era a terceira variante da pessoa | **`MenuDoParticipante` respondia volume, silencio local, mover, mudo/surdo e desconectar — sem cargos, sem apelido, sem castigo, sem expulsar, sem Copiar ID.** Agora e o `MenuDoUsuario` com um BLOCO a mais, e o bloco e so o que existe dentro de uma sala. ⚠ **O `serverId` passou a ser PARAMETRO**, e sem isso a unificacao introduziria um defeito: o menu lia `useServidorAtivo()`, mas uma chamada continua aberta enquanto a pessoa olha outro servidor — moderar agiria sobre o servidor errado, e numa chamada de DM sobre um que nao existe. ⚠ O bloco e componente proprio porque assina tres coisas (sala, volume, silencio) que nao existem fora da voz: com os hooks no menu, todo clique direito da timeline pagaria por elas. |
| Clique direito numa MENCAO abria o menu da mensagem | **Um atributo.** A mencao e o nome de uma pessoa no meio do texto, e o gesto sobre ela pergunta pela pessoa; `alvoNoDom` resolve o mais proximo, entao `data-menu-usuario` no botao da mencao ganha do `data-menu-mensagem` da linha. Medido: clique direito em `@Ana-vx` abre o menu do usuario. O mesmo atributo resolveu o `⋯` da tabela de Membros das configuracoes, **que abria DIRETO o modal de castigo** — um botao de "mais acoes" que executa UMA acao, a mais destrutiva das tres. |
| Silenciar tinha TRES formas e nenhuma marcava a escolha | **Canal: submenu com cinco prazos. Servidor: outro submenu com os mesmos cinco, escritos de novo. Conversa: um item so que alternava.** Nenhuma com ✓, nenhuma dizendo quanto falta (D-DMN-37, D-NOTIF-26/27). `menus/SubmenuDeSilenciar` e uma peca para as cinco superficies, sobre uma uniao marcada `canal \| servidor` — os dois mapas do store continuam separados porque a cadeia do design tem dois elos, e o menu nao precisa saber disso. ⚠ **O store passou a guardar a DURACAO escolhida junto do prazo**, e era o que faltava para o ✓: so com o prazo, vinte minutos depois de "Por 1 hora" e quarenta depois de "Por 8 horas" sao dois instantes futuros quaisquer. ⚠ **`silenciar()` novo, separado de `alternarSilencio`:** escolher "Por 8 horas" com o canal ja mudo e TROCAR o prazo, e pelo alternador o mesmo clique REATIVARIA — o contrario do que o item diz. Medido: `Silenciar canal · 1 h restantes` no gatilho, `Reativar avisos` primeiro, `aria-checked="true"` em "Por 1 hora". |
| ⚠ Categoria NAO ganhou silenciar, e a razao e de plumbing | Ela pediria um TERCEIRO elo na cadeia, e `estaMudo(channelId, serverId)` — quem responde "este canal esta mudo?" ao rollup e ao realce da coluna — nao recebe a categoria. O silencio existiria no menu e nao valeria em lugar nenhum, que e pior que a ausencia. O que ela ganhou foi **"Marcar categoria como lida"**, que e onde o item mais se paga: numa categoria de quinze canais, zera-los um a um sao quinze menus abertos. |
| Os atalhos do menu eram DESENHO | **`R`, `E`, `⌫` e `⇧⌘C` estavam escritos ao lado dos itens desde que o menu foi construido e nao faziam nada.** Um atalho anunciado que nao funciona e pior que atalho nenhum: quem tenta uma vez conclui que o app esta quebrado e nao tenta os outros. `menus/atalhosDaMensagem.ts` chama as MESMAS funcoes dos itens — dois caminhos para a mesma acao divergem no primeiro que ganha uma condicao, e aqui a condicao e permissao. ⚠ **O `preventDefault` e CONDICIONAL:** quando a acao nao e permitida (editar mensagem alheia), a tecla passa em vez de ser engolida em silencio. ⚠ **`Delete` alem de `Backspace`** — o menu desenha `⌫`, mas num teclado completo `Delete` e a tecla que a mao procura. Medido: `R` numa linha focada arma a resposta. |
| O glifo de atalho era `⇧⌘C` no Windows | **`lib/plataforma.ts`**, e ele tambem corrige a ORDEM: o Mac escreve `⇧⌘C` sem separador, o Windows escreve `Ctrl+Shift+C`. Isto existia escrito a mao em UM lugar (`TECLA_DA_PALETA`, na coluna de canais) e o menu, que tem quatro atalhos, nao consultava nada. Medido no Windows: `Copiar link · Ctrl+Shift+C`. |
| Reacoes rapidas eram curadoria do produto | **O design e explicito — *"emojis frequentes do usuario, nunca curadoria do produto"* — e o comentario do codigo admitia a divida em prosa:** *"frequencia por usuario e store que ainda nao existe"*. `store/reacoesFrequentes.ts` existe, persiste em `localStorage` e completa com a semente enquanto o historico for menor que quatro. ⚠ **Contagem e nao "ultimos usados":** uma pilha de recentes muda a ordem a cada reacao e as quatro posicoes viram alvo movel — quem aprendeu que o segundo e o do fogo erra no clique seguinte. ⚠ **So conta ao POR, nunca ao tirar:** contar o desfazer premiaria o emoji clicado por engano, duas vezes. |
| "Copiar link" saia da ROTA e nao da mensagem | **Nas fixadas, na busca, na caixa de entrada e no chat da sala de voz a rota e outra coisa que nao o canal da mensagem** — o link copiado apontava para o canal ABERTO com o id de uma mensagem que nao mora la. Quem cola descobre que nao funciona; quem copiou, nao. Agora sai de `message.channelId` + `serverId` do canal, e o mesmo vale para o atalho. As permissoes do menu ja usavam `message.channelId`; era so o link. |
| Menu de mensagem existia SO na timeline | **Fixadas, busca e caixa de entrada passaram a ter o mesmo.** Cada uma marca a linha com `data-menu-mensagem` e envolve a lista com o gatilho da onda 1.5A — sem componente novo, porque `MenuDaMensagem` ja lia o alvo do store. ⚠ **Na caixa de entrada o atributo vai na PREVIA e nao no cartao:** o cartao e sobre o CANAL (abrir, marcar lido) e a previa e sobre a mensagem, e marcar o cartao inteiro faria o clique direito no nome do canal abrir o menu de uma mensagem — a troca de entidade que esta onda veio consertar. O menu do canal ali fica pendente. |
| O foco nao voltava para a linha depois do `Esc` | **O Radix devolve ao GATILHO, e o gatilho aqui e a superficie inteira.** Na timeline ele e o container de dez mil linhas: abrir o menu com `Shift+F10` numa linha e fechar largava o foco na lista, e a pessoa perdia o lugar — medido, `document.activeElement` voltava com `role="log"`. Agora o `ContextMenuContent` devolve para onde o GESTO nasceu, consumido uma vez (guardar depois de devolver mandaria o foco para a linha do gesto anterior). Medido depois: volta para o `<article>` exato, com o mesmo `data-menu-mensagem`. |
| A guarda de menu e por ENTIDADE, com permissao cheia e vazia | **`menus/menus.test.tsx` renderiza cada menu com os primitivos DUBLADOS** — com os do Radix o teste precisaria de `Root`, `Anchor` e `Portal` e passaria a medir o Radix; o que ele mede e a lista que nos escrevemos. Quatro regras gerais valem para todos: `ItemDeId` por ultimo e unico, perigo no fim DO BLOCO dele, menu nunca vazio, separador nem duplo nem no topo. ⚠ **A regra de perigo nasceu mais forte — "o ultimo do menu" — e o teste a desmentiu no primeiro menu de DM:** o design poe "Desfazer amizade" e "Bloquear" em vermelho num bloco e "Fechar conversa" NUM BLOCO DEPOIS, sem ser vermelho. Duas mutacoes verificadas (permissao ignorada, `ItemDeId` fora do fim). |
| ⚠ `window` no teste e LOCAL, e de proposito | `react-dom/client` precisa dele (le `window.event` para decidir a prioridade do update), e `src/testes/documento.ts` instala so `document`, `localStorage` e `fetch` com a doutrina escrita: *"instala a unica peca que faltava e nao mexe em mais nada"*. Por `window` no setup global mudaria o ramo que todo modulo com `typeof window` escolhe, e o sintoma seria uma suite distante trocando de caminho sem ninguem notar. || ⚠ O Radix suprime o menu NATIVO, e com ele sumiam seis ações | **A quebra nº 7, e ela não aparece em código nenhum — aparece no que DEIXA de existir.** Onde a página chama `preventDefault` no `contextmenu`, o Chromium não abre o menu dele: copiar seleção, copiar endereço do link, abrir link, copiar imagem e salvar imagem existem em todo navegador e **em nenhum ponto deste app**. `store/pontoDoMenu.ts` resolve o que está SOB O PONTEIRO no mesmo gesto em que o alvo é mirado, e a linha ganha o bloco condizente. Medido no arnês: sobre `<a download>` entra **"Copiar link do arquivo"**, sobre `<a href>` entram **"Abrir link" e "Copiar endereço do link"**, sobre `<img>` entram **"Copiar imagem" e "Salvar imagem"**, e no texto liso o menu segue com os 15 de sempre. ⚠ **`download` e não a extensão:** anexo e link de texto são os dois um `<a href>`, e "Abrir link" num `.pdf` é baixá-lo com outro nome. |
| ⚠ "Copiar seleção" precisa do `contains`, e do `isCollapsed` | **Selecionar num lugar e clicar com o direito noutro é o gesto mais comum de quem lê rolando** — sem o teste de contenção o item apareceria no menu de uma mensagem oferecendo o texto de OUTRA, e o erro só apareceria na colagem. E um clique simples deixa uma seleção de comprimento ZERO ancorada onde se clicou, que passa no `contains`: sem `isCollapsed` o item entraria em todo clique direito, copiando nada. Medido em navegador: com a seleção na linha 4, o menu DELA traz "Copiar seleção" e o da linha 7 não. |
| ⚠ Copiar imagem passa por `canvas`, e o `ClipboardItem` recebe a PROMESSA | Duas armadilhas de plataforma numa ação só. (1) A área de transferência aceita um conjunto curto de tipos e `image/png` é o único universal — escrever o `image/webp` que o `autumn` devolve é recusado com `NotAllowedError`, então o blob é redesenhado e reexportado. (2) O Safari exige que `write` seja chamado ainda DENTRO do gesto; com `await` antes, o gesto expirou e a escrita é negada — passar a promessa mantém a chamada síncrona em relação ao clique. `bitmap.close()` porque uma imagem de 4000×3000 decodificada são ~48 MB e esperar o coletor é o erro nº 5 para quem copia várias seguidas. |
| ⚠ O menu nativo da casca abria em QUALQUER lugar, com um item só | **"Toggle spellcheck" entrava incondicionalmente**, então a guarda `menu.items.length > 0` era sempre verdadeira: clicar com o direito no vão da janela abria uma caixa com um ajuste que ninguém procurava ali — em inglês, num app em português. E o inverso era pior: **campo de texto não tinha recortar, copiar nem colar**, e num app Electron o menu do navegador não existe, ou seja não havia como colar com o mouse. `native/menuNativoModelo.ts` é puro e testado (16 casos): lista VAZIA significa "não abra nada", edição sai de `params.editFlags` (um campo somente-leitura tem `canPaste: false`), e o corretor só existe onde se digita. ⚠ **"Abrir link" só para o que `abrirNoNavegadorDoSistema` aceita** — a mesma função do `setWindowOpenHandler`: passar um `file:` a `shell.openExternal` é entregar ao sistema uma URL escrita por outra pessoa. Copiar continua valendo, porque copiar não executa. |
| ⚠ O corretor era gravado por um e lido por ninguém | **`config.spellchecker` existia no store da casca desde o upstream, o menu o alternava, `config.sync()` mandava a chave ao renderer — e nenhuma tela a mostrava, nem a partida a reaplicava.** Quem ligava o motor era o `webPreferences` da janela, que decide na criação. Agora `aplicarCorretor()` roda na partida e a opção existe em Configurações › Desktop, em seção própria ("Digitação"), **sem selo de reinício porque vale na hora**. ⚠ **`setSpellCheckerLanguages` jamais tinha sido chamado**, então o corretor ficava em inglês num app em português: TODA palavra sublinhada, que é o mesmo que não ter corretor com o custo de riscar a tela. O idioma sai do sistema, FILTRADO pela lista disponível — o método lança com um código que a sessão não conhece, e uma exceção no caminho de partida derrubaria a janela por causa de um corretor. ⚠ **`en-US` de RESERVA e não de acréscimo:** somá-lo ao português faria o corretor validar contra a união dos dicionários e aceitar palavra inglesa no meio do texto. |
| ⚠ A chave do corretor muda de nome na fronteira, e o teste exige as duas coisas | O store da casca é o do upstream (`spellchecker`) e o cliente grava em português (`corretorOrtografico`). A tradução é o trabalho de `preferenciasDoCliente.ts` — e o teste que já existia, *"recusa as chaves do upstream"*, continua exigindo que `spellchecker` CRU seja rejeitado: sem isso o renderer, que executa conteúdo de terceiro, escreveria direto no arquivo que o main lê antes de a janela existir. |
| Moderação em voz sem aviso nenhum | **Resolvida, e os três casos sumiam em silêncio.** Ser MOVIDO de canal chegava aqui como `RoomEvent.Disconnected` do LiveKit e o motor respondia zerando a chamada — faixa e cartão sumiam e nada dizia por quê; ser DESCONECTADO dava o mesmo quadro; ser SILENCIADO acendia o `SRV` na linha dos OUTROS e não dizia nada a quem perdeu o microfone. ⚠ **A corrida é o miolo, e ela vem de o servidor emitir DOIS sinais por caminhos diferentes:** `member_edit.rs` chama `remove_user` (o LiveKit fecha com `PARTICIPANT_REMOVED`) e só então publica `UserMoveVoiceChannel` pelo websocket. Reagir na chegada do primeiro diria "você foi desconectada" e, meio segundo depois, "você foi movida" — dois avisos contraditórios sobre o mesmo fato. `sdk/vozImposta.ts` abre uma janela de 1,5s: quem chega primeiro espera o segundo, e MOVIMENTO ganha de remoção, porque a remoção que acompanha um movimento é o transporte dele. Módulo puro (não importa `livekit-client` nem o adapter), 11 testes com relógio falso, duas mutações verificadas. ⚠ **`UserMoveVoiceChannel` é lido do evento CRU:** o `case` dele em `events/v1.ts` é um `// todo` vazio e o tipo do SDK nem declara `from`/`to`, que o servidor manda. ⚠ **Não dizemos QUEM moveu, e o design pede** ("por Ana Ribeiro"): nenhum dos dois sinais carrega o autor, ele só existe no registro de auditoria — e ir buscá-lo seria uma chamada de rede no instante do aviso, sobre uma rota que exige `ViewAuditLog`, permissão que justamente quem acabou de ser movido quase nunca tem. |
| Arrastar para mover em voz | **Construído, e é afordância de PONTEIRO — o submenu continua sendo o caminho de teclado e de toque**, a mesma decisão já registrada para as pastas do rail. ⚠ **O veredito é calculado no `dragenter`, nunca no `dragover`:** o navegador dispara o segundo a cada ~50ms com o ponteiro parado, e escrevê-lo no store acordaria a coluna a 20Hz. ⚠ **O overlay assina, a linha do canal não** — `AlvoDeSoltura` é componente próprio pela mesma razão do `TetoDaSala`, e o snapshot é booleano, então entrar num alvo acorda DUAS linhas e nenhuma outra. Medido em navegador: alvo válido com borda tracejada `rgb(53,194,204)` e "soltar Bruno aqui", cheio com `rgb(232,89,107)` e "canal cheio · não aceita soltura", `dragover` com `preventDefault` só no válido (o cursor de recusa é do navegador), e o `drop` chamando `moverParaCanalDeVoz`. ⚠ **`Connect` é permissão de QUEM MOVE**, e parece errado: `member_edit.rs` calcula as permissões do canal de destino sobre o moderador. Entrou como `conectar` na união `Acao`. |
| Canal de voz cheio: clicar não tenta mais entrar | **Resolvido.** O servidor já barrava (`voice_join.rs` devolve `CannotJoinCall`), mas o caminho até lá era pedir o nó, pedir o token e ouvir não — com toast de erro no fim de uma ida e volta. Agora a sala cheia ABRE O CHAT, que é o que o design manda (*"continua clicável para abrir chat embutido"*), e o selo `CHEIO` ao lado já diz por quê. ⚠ **`ManageChannel` fura o teto no servidor e aqui NÃO**, de propósito: o teto é escolha de quem configurou a sala, e duas regras diferentes para o mesmo teto — uma no clique e outra no arraste — seriam piores que a divergência contra o servidor. Medido: com a sala em 2/8 o canal ativo não muda (entrou na chamada); com 3/3 o canal ativo passa a ser ele (abriu o chat). |
| Arnês mais pobre que o protocolo — 12ª vez | **`can_receive` nunca tinha sido semeado.** `surdoPeloServidor` entrou no `ParticipanteDeVoz` quando o menu do participante precisou saber se o item estava marcado, e NENHUMA pessoa do firehose o tinha em `true`: campo lido, mapeado, com consumidor no menu, e invisível na coluna — o ícone nasceria inalcançável, como o `SRV` teria nascido sem `semearMudoDoServidor`. Agora `semearSurdoDoServidor` existe e o rig marca uma pessoa, em outra sala e outra pessoa que o mudo (com os dois na mesma linha nunca se veria o caso de um só, que é o mais comum). Medido: `Bruno · SRV · ensurdecido pelo servidor`. |
| ⚠ O arnês não alcança hierarquia, então arraste e submenu de mover são INVERIFICÁVEIS nele | **Dito, e é anterior a esta rodada.** `abaixoDeMim` sai de `inferiorTo(eu)`, que precisa do meu `ServerMember` — sem sessão ele é `false` para todo mundo, e com ele caem o arraste, o bloco de moderação do menu do participante e o submenu "Mover para canal". Verifiquei os dois com SONDA TEMPORÁRIA (trocando `abaixoDeMim === true` por `membro !== undefined`), medindo, e revertendo — está registrado porque medir com o código alterado é exatamente o que este arquivo cobra em outros lugares. O conserto de verdade é o rig ter usuário local com cargo, que é trabalho próprio. |
