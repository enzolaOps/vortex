...
| Campos de protocolo ainda ignorados | **Sobraram dois dos treze.** `havePermission` virou REGRA e está adotado — seis alvos perguntam antes de existir, e a fase 6 liga o SDK num lugar só. Faltam **`muted`** (silenciar canal escreve no protocolo) e **`banner`** (precisa da superfície de perfil de servidor, que não existe). |
