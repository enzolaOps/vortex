...
| ⚠ 43 props de tamanho de ícone são MORTAS | **Anterior ao meu passe, e a escala nomeada foi o que tornou visível.** Medido em navegador na tela inicial: 43 ícones trazem `size={20}` no TSX e desenham **9, 12 ou 14**, porque uma regra como `.acoes svg { inline-size: var(--vx-icon-1) }` vence a prop. O número escrito não é o da tela — é a família do comentário que afirma uma medida que não existe. Os valores RENDERIZADOS estão quase todos certos (12 e 14 são degraus); o que está errado é o TSX mentir. Limpar exige varrer os ~17 seletores vencedores decidindo em cada um quem deve ser o dono, e alguns ficam em arquivo sendo editado em paralelo. |
| 75 usos de ícone sem `size` nenhum | **Renderizam no `1em` do Phosphor — ou seja, o tamanho deles é o da fonte herdada, que é o oposto de uma escala.** Medido: 40 na barra de ações do hover (13px, herdado do corpo da mensagem) e 2 no caret de categoria. ⚠ **O caret sai a 9px**, abaixo de todo degrau das duas escalas, por `inline-size: 9px` escrito à mão em `.chevron`. Não corrigido porque **o design não decide**: os `.dc.html` não desenham `<svg>` com tamanho, então trocar 9 por 12 seria escolha minha sem oráculo. 33 dos 75 estão em `MessageRow.tsx`, editado em paralelo. |
