...
| Estimativa de altura de linha | **Promovida de ajuste a suspeito nº 1, e em teste.** `estimateSize: () => 44` nunca foi medido — foi chutado na fase 0, antes de existir agrupamento, divisor de data e estado de envio. Medição direta no DOM: média **82,4px**, mínimo 52, máximo 129. Cada linha que monta chega ~38px acima do estimado, o total do virtualizador salta e a âncora compensa — no mesmo frame do append, que é o frame lento. O arnês ganhou chave de A/B (44 contra 73) e passou a reportar a altura real medida. |
