...
| Custo de pintura | **Hipótese enfraquecida, e a evidência que a sustentava caiu.** Ela nasceu da diferença entre 0,4–0,5% em headless e 5,4–6,3% em display real, atribuída a rasterização. Metade dessa evidência era instrumento: o lado de 5,4–6,3% vinha de corridas com vazão degradada e estimador quebrado. Hoje o headless mede 1,7% com carga cheia. Refazer o A/B em display real com as duas guardas em vigor antes de tratar pintura como causa — e antes de reabrir a remoção da máscara do ponto de presença. |
...
| Gate depois do passe de design | **PASS, e desta vez o número vale.** Mediana de 3 sob CPU 4x com a máquina limpa: **1,7% · 1,7% · 1,9%** de frames perdidos contra o teto de 5% — espalhamento de 0,2pp, p95 idêntico nas três (12,4ms · 1,97× o refresh), zero long tasks, 94,6–94,7% dos frames num intervalo único, vazão de 477–481 de 500 ev/s. O estimador acertou o refresh em 6,3ms nas três. Comparável às 1,4/1,5/1,7 de antes do passe de design: **a barra de ações no hover, o limite de erro por painel e o divisor sem régua não custaram frame.** |
...
| Campos de protocolo ainda ignorados | **Sobraram três dos treze.** Consumidos na fase 5: `reactions`, `replyIds`, `pinned`, `roleColour`, `editedAt`, `systemMessage`, `channel.description`, `pronouns`, `timeout`, `status.text`. Faltam **`muted`** (silenciar canal — escreve no protocolo, fase 6), **`havePermission`** (é REGRA e não item: nunca renderizar ação que a pessoa não pode executar; custa zero adotada agora e é varredura depois) e **`banner`** (precisa da superfície de perfil de servidor, que não existe). |
