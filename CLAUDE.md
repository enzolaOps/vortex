...
| Gate reprovando a MÁQUINA | **Achado rodando o gate, e mecanizado.** Uma corrida deu FAIL com 6,6% de frames perdidos, vazão de 414/500 e uma população de deltas sub-vsync — enquanto um processo fora do projeto (um jogo) consumia **512% de CPU**. Nada naquele relatório era sobre o código. O check de ambiente só reprovava "headless não compõe frames", que passa numa máquina com cinco núcleos ocupados. Agora ele exige que ≥85% dos frames EM REPOUSO caiam num intervalo único. **A régua é o vsync, nunca a mediana** — a primeira versão media contra a própria mediana dos deltas, que numa máquina disputada já vem inflada (11,1ms num display de 164Hz), e passou justamente na corrida que existia para reprovar. |
