...
| Cargos e permissões | **Construída — era a tela mais densa do plano** (no upstream, um editor de bitmask de 596 linhas). O que a torna administrável é a tradução: `PERMISSOES` agrupa por pergunta que alguém faz de fato ("quem pode expulsar?"), cada item diz a CONSEQUÊNCIA em vez de repetir o rótulo, e **nenhum `BigInt` chega ao componente**. ⚠ **`BigInt` e não `number` na conversão**, e isso não é preciosismo: voz e menção moram nos bits 30–39, e os operadores bitwise do JavaScript truncam em 32 — `Speak` (bit 31) viraria negativo e `MentionRoles` (bit 38) sumiria. Erro que só aparece no fim da lista. ⚠ **A linha "sem arrastar, porque `DataEditRole.rank` não tem efeito" ENVELHECEU, e o erro estava na conclusão e não no fato.** O campo continua documentado no protocolo como *"**Removed** - no effect"*; quem reordena é `PATCH /servers/:id/roles/ranks`, que `reordenarCargos` já chamava por `setRoleOrdering` desde que a tela existe. O que faltava não era a escrita — era a TRAVA. O servidor recusa com `NotElevated` qualquer reordenação que MUDE A POSIÇÃO de um cargo no seu nível ou acima (`roles_edit_positions.rs`), e a tela não conhecia a regra: ela reordenava otimista, tomava o erro e recarregava, ou seja produzia exatamente o "parece funcionar e não salva" que a decisão de não arrastar dizia estar evitando. `reordenacaoPermitida` espelha a regra do servidor, e a alça passou a arrastar (fantasma a 60%, linha de acento no destino), com o mesmo `mover` do teclado. ⚠ **Uma regra só sobre o cargo MOVIDO não serve, e é o caso que o teste guarda:** subir um cargo abaixo de mim até o topo EMPURRA os de cima, e o servidor recusa. |
| Emojis: listar e apagar, não enviar | ⚠ **A ausência é dita, e tem razão.** Subir emoji não passa pela API do protocolo: é um `POST` cru para o servidor de MÍDIA, com URL vinda de `client.configuration.features.autumn`. Sem instância alcançável não há como escrever e ver funcionar — e a única coisa pior que não ter o botão é ter um que falha em silêncio contra um endpoint que ninguém testou. ⚠ **"O protocolo também não tem editar emoji" estava ERRADO, e três lugares do repositório o repetiam.** `PATCH /custom/emoji/{id}` existe: está montada em `customisation/mod.rs` (`emoji_edit::edit_emoji`), consta do OpenAPI do `stoat-api`, recebe `DataEditEmoji { name }`, exige `ManageCustomisation` e grava `EmojiUpdate` na auditoria. O ID **não muda** no PATCH — e é o ID que as mensagens antigas referenciam, então nada quebra; o que exige emoji novo é trocar a IMAGEM, não o nome. O alias agora é editável na linha, salvando ao sair do campo. ⚠ **A unicidade é guardada pelo CLIENTE**: `emoji_edit.rs` só valida o formato, então dois `:festa:` no mesmo servidor são graváveis e tornam o seletor ambíguo. Colisão não escreve — borda `danger`, o dono do alias ao lado e `role="alert"`, porque o campo já perdeu o foco quando o veredito sai. |
...
| Três colunas de Convites e duas de Banimentos | ⚠ **Não existem no protocolo, e eu tinha afirmado o contrário.** Na auditoria de páginas eu disse que "criador, usos e expira existem em `Invite`". Só **`creator`** existe: `uses`, `max_uses`, `expires_at`, `temporary` e `vanity` dão **zero** ocorrências no schema do `stoat-api`. Em `ServerBan` é pior — ele tem `_id`, `reason` e o usuário, então "Banido por" e "Data" também não são campos. As cinco ficaram de fora em vez de virar coluna com traço, e o rodapé de cada página diz isso. ⚠ **As duas de banimento EXISTEM agora**, e vêm de `/servers/{id}/audit_logs` com `type=BanCreate`, como esta linha previa. A primeira ocorrência de cada conta ganha (o servidor ordena por `_id` decrescente), então quem foi banido, perdoado e banido de novo mostra o banimento que está valendo. ⚠ **Segunda consulta e não `Promise.all`:** `ViewAuditLogs` é permissão separada de `BanMembers`, e encadeá-las faria quem só pode desbanir ver a página inteira falhar por causa de duas colunas — ela falha em silêncio e a tabela chega com a primeira resposta. ⚠ **Ausência é DITA:** a auditoria guarda uma janela, não a história, então banimento fora dela diz "sem registro" em vez de deixar a célula vazia, que é indistinguível de "ninguém assinou". O filtro "Todos os moderadores" do design entrou junto — o comentário que explicava por que ele não podia existir ("sem saber quem baniu, não teria por onde filtrar") caiu com as colunas. |
...
| A superfície ausente de maior valor: registro de auditoria | **CONSTRUÍDA — esta linha dizia "é a única que falta", e envelheceu duas vezes.** A tela existe (`config/Auditoria.tsx`), e agora a auditoria é FONTE para outras duas superfícies, que é o que a linha previa: as colunas "Banido por" e "Data" de Banimentos, e o histórico da pessoa dentro do modal de moderação. ⚠ **O filtro de PERÍODO vai ao servidor e os outros três não**, e a divisão tem razão: autor, ação e busca reordenam o que já está na tela; o período decide o que a consulta traz. A rota pagina por ID de entrada e não por data, então `idDeInstante` monta o piso de ULID — o filtro do Mongo é `_id > x` e ULID começa com o tempo em base32, o que torna "últimos 90 dias" exprimível. ⚠ **`type` vai como UMA string e nunca um array:** o cliente do `stoat-api` monta a query com um `append` por chave, então um array viraria `type=A,B` e o Rocket leria um tipo só chamado "A,B" — um filtro que silenciosamente não acha nada é pior que não ter filtro. |
...
| Cargo de INTEGRAÇÃO não é representável | ⚠ **O design trava com 🔒 os "cargos gerenciados por integração", e o aviso da coluna de cargos os prometia — sem que o protocolo os tenha.** `Role` carrega `_id`, `name`, `permissions`, `colour`, `hoist`, `rank`, `icon` e o `mentionable` do Vortex, e nada que diga "gerido por um bot"; medido no `servers.rs` dos dois crates e no schema do `stoat-api`. Prometer uma trava que nenhum servidor sabe produzir é a família do comentário que afirma uma medida que não existe, e a frase saiu do aviso. O cadeado que a coluna mostra é o de HIERARQUIA, que é real. Fica FORA do `pendencias.ts` pelo contrato dele — não há controle para clicar, mesma família da etiqueta FÓRUM e da reação SUPER. |
| A opacidade 0,6 do item travado fica na ALÇA, não na linha | ⚠ **Divergência 1:1 deliberada, e a razão é a garantia de contraste.** O design escurece o item travado inteiro; aqui o nome do cargo sai na cor do servidor passada pelo clamp de L, e `opacity` num ancestral multiplica a luminância DEPOIS do clamp — a garantia de 4,5:1 cairia sem nada acusar, porque o `pnpm contrast` lê tokens e não composição de camada. O cadeado, o cursor e a contagem esmaecida carregam o estado; a legibilidade fica. |
| ⚠ O arnês mais GENEROSO que o protocolo — 12ª vez, e a primeira nessa direção | **As onze anteriores foram o arnês entregando de MENOS.** Esta foi o contrário: o envelope de `/audit_logs` em `dev/rede.ts` ignorava `type` e `target` e devolvia as cinco entradas sempre. A tela de Banimentos consulta com `type=BanCreate` e teria funcionado **por acidente** — ela receberia tudo e acharia o banimento no meio —, enquanto contra o servidor real o filtro corta e o número devolvido muda. Ele passa a filtrar e a **ordenar por `_id` decrescente como o Mongo**: sem a ordem, a régua de dia saiu com um "16 DE SETEMBRO" acima de um evento das 19h daquele mesmo dia, porque ela só compara com a linha anterior. Entraram também um `MemberEdit` com `roles` (o único caminho até a frase específica de D-LAC-18, que nunca tinha chegado à tela) e `target` nos banimentos — dois dos três, de propósito, porque "banimento fora da janela do registro" é um estado que a tabela precisa dizer. |
