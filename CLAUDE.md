...
| Carregamento progressivo (janela deslizante) | **Ideia levantada, com mecanismo identificado e não medida.** Hoje o canal inteiro fica na lista, e a publicação da coleção copia o array de IDs inteiro por frame com mensagem nova — `[...idsOf(channelId)]`, medido em 0,57ms por publicação a 10k. É custo O(total) num caminho que só precisa da janela. Uma janela deslizante à la Discord o tornaria O(janela), e limitaria junto o `measurementsCache` do virtualizador e a memória de sessão de 8h. **Testar antes de construir:** semear 1.000 em vez de 10.000 e ver se o p95 cai um degrau. Se cair, o total é o driver; se não, a janela resolve memória e não resolve o gate. |
